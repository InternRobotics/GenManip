...
if __name__ == "__main__":
    args = parse_args()
    data = get_all_data(args.data_path, args.camera_name)
    ...
    if len(data["color_image"]) == 1:
        ...
        cv2.imwrite(args.output_path, data["color_image"][i])
        exit()
    fourcc = cv2.VideoWriter_fourcc(*"mp4v")
    video_writer = cv2.VideoWriter(f"{args.output_path}", fourcc, 30, (640, 480))
