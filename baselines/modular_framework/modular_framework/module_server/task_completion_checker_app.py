...
import traceback
import cv2
from utils.prompt import construct_gpt_completions_tracking_prompt
...
def process_request():
    try:
        ...
        try:
            ...
            logger.error(f"Failed to convert image data to numpy array: {e}")
            logger.debug(traceback.format_exc())
            return jsonify({"error": "Invalid image data format."}), 400
        if not instruction or not image_data:
            return jsonify({"error": "Instruction and image data are required."}), 400

        prompt = construct_gpt_completions_tracking_prompt(instruction)
...
if __name__ == "__main__":
    app.run(host="0.0.0.0", port=5009, threaded=False, processes=64)
