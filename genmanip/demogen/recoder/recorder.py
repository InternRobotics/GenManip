...
def collect_task_data(object_list, franka_list, camera_data, task_data, usd_path_list):
    ...
    for franka in franka_list:
        task_data["initial_layout"][franka.robot.name] = {}
        task_data["initial_layout"][franka.robot.name]["position"] = franka.robot.get_world_pose()[
            0
        ]
        task_data["initial_layout"][franka.robot.name][
            "orientation"
        ] = franka.robot.get_world_pose()[1]
        task_data["initial_layout"][franka.robot.name][
            "joint_positions"
        ] = franka.robot.get_joint_positions()
...
class Logger:
    ...
    def load_static_info(self):
        for camera_name, camera in self.cameras.items():
            ...
            # if "id2labels" in camera_info:
            #     self.add_json_data(
            #         f"observation/{camera_name}/id2labels",
            #         camera_info["id2labels"],
            #     )
            self.add_json_data(
                f"observation/{camera_name}/camera_params",
                camera_info["intrinsics_matrix"].tolist(),
            )
        self.add_json_data(
    ...
    def load_dynamic_info(self, action, gripper_close, object_list=[], name=None):
        ...
        self.add_scalar_data(
            f"ee_pose_action", joint_position_to_end_effector_pose(action)
        )
        ...
        for camera_name, camera in self.cameras.items():
            ...
            # if "motion_vectors" in camera_info:
            #     self.add_scalar_data(
            #         f"observation/{camera_name}/motion_vectors", camera_info["motion_vectors"]
            #     )
            self.add_scalar_data(
        ...
        for key in object_list:
            self.add_scalar_data(
                ...
                object_list[key].get_world_pose()[0],
            )
            self.add_scalar_data(
                f"observation/obj_pose/{key}/orientation",
                object_list[key].get_world_pose()[1],
            )
            self.add_scalar_data(
                f"observation/obj_pose/{key}/scale", object_list[key].get_local_scale()
            )
        ...
        self.add_scalar_data(
            ...
            joint_position_to_end_effector_pose(self.franka.get_joint_positions()),
        )
        self.add_scalar_data(
    ...
    def save(self, task_name=None, config_path=None):
        ...
        for key, value in self.color_image_logger.items():
            root_img_path = self.log_dir / f"{key}"
            ...
            create_video_from_image_array(
                value, os.path.join(root_img_path, "demo.mp4"), fps=30
            )
